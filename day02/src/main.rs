...
use crate::Strategy::{PAPER, SCISSOR, ROCK};
...
impl Duel {
    fn create(elf_raw: &str, own_raw: &str) -> Duel {
        ...
        Duel { elf_strategy, own_strategy }
    }

    fn play(&self) -> u32 {
        match &self.own_strategy {
            SCISSOR => SCISSOR_POINTS + match &self.elf_strategy {
                SCISSOR => DRAW_GAME_POINTS,
                PAPER => WON_GAME_POINTS,
                ROCK => LOST_GAME_POINTS
            },
            PAPER => PAPER_POINTS + match &self.elf_strategy {
                SCISSOR => LOST_GAME_POINTS,
                PAPER => DRAW_GAME_POINTS,
                ROCK => WON_GAME_POINTS
            },
            ROCK => ROCK_POINTS + match &self.elf_strategy {
...
impl DuelWithOutcome {
    fn create(elf_raw: &str, own_raw: &str) -> DuelWithOutcome {
        ...
        DuelWithOutcome { elf_strategy, outcome }
    }
    ...
    fn play(&self) -> u32 {
        ...
        let duel = Duel { elf_strategy, own_strategy };
        duel.play()
    }
}
